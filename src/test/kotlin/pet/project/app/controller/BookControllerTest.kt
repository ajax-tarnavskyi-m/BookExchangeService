...
class BookControllerTest {
    ...
    private lateinit var updateAmountRequests: List<UpdateAmountRequest>
    ...
    fun setUp() {
        ...
        updateAmountRequests = listOf(
            UpdateAmountRequest(bookId = "60f1b13e8f1b2c000b355777", delta = 5),
            UpdateAmountRequest(bookId = "60f1b13e8f1b2c000b355778", delta = -2)
        )
    }
    ...
    @Test
    fun `updateAmount should return total updated amount`() {
        // GIVEN
        every { bookService.changeAmount(updateAmountRequests) } returns 7

        // WHEN
        val resultActions = mockMvc.perform(
            patch("/book/amount")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateAmountRequests))
        )

        //THEN
        resultActions.andExpect(status().isOk)
        resultActions.andExpect(content().string("7"))
    }
