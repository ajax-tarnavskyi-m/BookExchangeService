...
class UserControllerTest {
    ...
    private lateinit var createUserRequest: CreateUserRequest
    private lateinit var updateUserRequest: UpdateUserRequest
    private lateinit var responseUserDto: ResponseUserDto
    private lateinit var initializedUser: User
    ...
    @BeforeEach
    fun setUp() {
        createUserRequest = CreateUserRequest("testUser", emptySet())
        updateUserRequest = UpdateUserRequest("66c35b050da7b9523070cb3a", "updatedUser", dummyWishlist)
        responseUserDto = ResponseUserDto("66c35b050da7b9523070cb3a", "testUser", emptySet())
        initializedUser = User(ObjectId("66bf6bf8039339103054e21a"), "testUser", emptySet())
    }
    ...
    fun `create user successfully`() {
        ...
        every { userService.create(user) } returns initializedUser
        ...
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.bookWishList").isEmpty)
    ...
    fun `get user by id successfully`() {
        // GIVEN
        val user = User(ObjectId("66bf6bf8039339103054e21a"), "testUser", emptySet())

        every { userService.getById("66c35b050da7b9523070cb3a") } returns user

        // WHEN
        mockMvc.perform(get("/user/{id}", "66c35b050da7b9523070cb3a"))
    ...
    fun `update user successfully`() {
        // GIVEN
        val user = updateUserRequest.toModel()
        ...
        mockMvc.perform(
            ...
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.login").value(updateUserRequest.login))
            .andExpect(jsonPath("$.bookWishList").isNotEmpty)
